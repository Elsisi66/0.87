...
def _random_genome(rng: random.Random, mode: str) -> Dict[str, Any]:
    tight = str(mode).lower() == "tight"
    g = {
        "entry_mode": rng.choice(ENTRY_MODES),
        "limit_offset_bps": rng.uniform(0.0, 30.0),
        "max_fill_delay_min": rng.randint(0, 45 if tight else 180),
        "fallback_to_market": rng.choice([0, 1]),
        "fallback_delay_min": 0,
        "max_taker_share": rng.uniform(0.0, 0.25 if tight else 1.0),
    ...
    return _repair_genome(g, mode=mode)


def _repair_genome(g: Dict[str, Any], mode: str) -> Dict[str, Any]:
    out = dict(g)
...
def _mutate_genome(g: Dict[str, Any], rng: random.Random, mut_rate: float, mode: str) -> Dict[str, Any]:
    out = dict(g)
    keys = list(out.keys())
    for k in keys:
    ...
    return _repair_genome(out, mode=mode)


def _crossover(a: Dict[str, Any], b: Dict[str, Any], rng: random.Random, mode: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    keys = sorted(a.keys())
    if rng.random() < 0.5:
        # Uniform crossover.
        c1: Dict[str, Any] = {}
        ...
        for k in keys[lo : hi + 1]:
            c1[k], c2[k] = c2[k], c1[k]
    return _repair_genome(c1, mode=mode), _repair_genome(c2, mode=mode)
...
def run(args: argparse.Namespace) -> Path:
    ...
    fee_payload = {
        "generated_utc": _utc_now().isoformat(),
        "source_script": "src/execution/ga_exec_3m_opt.py",
    ...
    _json_dump(run_dir / "fee_model.json", fee_payload)
    (run_dir / "metrics_definition.md").write_text(_metrics_definition_text(), encoding="utf-8")
    ...
    if str(args.resume).strip() and resume_path.exists():
        ...
        population = [
            _repair_genome(dict(g), mode=str(args.mode))
    ...
    if not population:
        population = [_random_genome(rng=rng, mode=str(args.mode)) for _ in range(int(args.pop))]
    ...
    for gen in range(int(start_gen), int(args.gens)):
        ...
        while len(new_pop) < max(0, len(population) - imm_n):
            ...
            c1, c2 = _crossover(p1, p2, rng=rng, mode=str(args.mode))
            c1 = _mutate_genome(c1, rng=rng, mut_rate=mut_rate, mode=str(args.mode))
            c2 = _mutate_genome(c2, rng=rng, mut_rate=mut_rate, mode=str(args.mode))
            new_pop.append(c1)
            if len(new_pop) < max(0, len(population) - imm_n):
                new_pop.append(c2)

        while len(new_pop) < len(population):
            new_pop.append(_random_genome(rng=rng, mode=str(args.mode)))
