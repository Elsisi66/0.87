...
def _attach_universe_features(df: pd.DataFrame, feat_table: pd.DataFrame) -> pd.DataFrame:
    ...
    m = pd.merge_asof(
        ...
        feat_table.sort_values("signal_time"),
        on="signal_time",
        direction="backward",
    )
...
def run(args: argparse.Namespace) -> Path:
    ...
    contract = {
        ...
        "entry_semantics": {
            "v2r_1h_reference": "1h_reference_control_path",
            "v3r_exec_control": "baseline_entry_path_from_exec_engine",
            "v4r_exec_phasec_best": "same_entry_as_v3r_with_phasec_best_exit",
        },
    ...
    representative_enough = int(
        ...
        and rep_tvd_mean <= phasec_tvd_mean * 0.90
        and np.isfinite(rep_cov)
        and rep_cov >= 0.70
        and np.isfinite(rep_max_delta_key)
        and rep_max_delta_key <= 0.35
    )
    ...
    t_v2r = phasec_bt._simulate_1h_reference(
        signals_df=rep_signals_eval,
    ...
    representativeness_gates = {
        ...
        "no_catastrophic_bucket_undercoverage": int(np.isfinite(rep_max_delta_key) and rep_max_delta_key <= float(args.max_bucket_delta)),
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--max-bucket-delta", type=float, default=0.35)
