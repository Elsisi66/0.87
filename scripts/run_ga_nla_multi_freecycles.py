...
DOES NOT force cycle 3.
Also expands GA cycle choices to any subset of regimes 0..4 (size 1..3).
Saves to <SYMBOL>_NLA (separate from normal).
"""
...
def _load_seed(symbol: str) -> Dict[str, Any]:
    ...
    # Prefer symbol's active/seed; fallback to BTC active
    for fp in [
...
def compute_cycles_no_lookahead(df: pd.DataFrame, p: Dict[str, Any]) -> np.ndarray:
    ...
    n = len(df)
    cycles = np.full(n, 2, dtype=np.int8)   # default = accumulation(2)
...
def run_backtest_long_only_no_lookahead(
    ...
    """
    Same bar model as GA backtest, but:
    - cycles use t-1 info
    - RSI-exit profit check uses OPEN (decision-time) not CLOSE
    """
    df = df.copy()
    df["Timestamp"] = pd.to_datetime(df["Timestamp"], utc=True, errors="coerce")
    df = df.dropna(subset=["Timestamp"]).sort_values("Timestamp").reset_index(drop=True)

    sig = np.asarray(g.build_entry_signal(df, p), dtype=bool)
...
def main():
    ...
    # Patch GA core to NLA-safe versions
    g.compute_cycles = compute_cycles_no_lookahead
    g.run_backtest_long_only = run_backtest_long_only_no_lookahead

    # Expand cycle search space: choose random subset from 0..4 (size 1..3)
    def _random_trade_cycles_full() -> List[int]:
        universe = [0, 1, 2, 3, 4]
        k = random.choice([1, 2, 3])
        return sorted(random.sample(universe, k))
    g._random_trade_cycles = _random_trade_cycles_full

    for base_symbol in base_syms:
        out_symbol = base_symbol + args.suffix
        print(f"\n=== NLA GA (FREE CYCLES): {out_symbol} (data={base_symbol}, tf={args.tf}) ===", flush=True)

        df = _load_df(base_symbol, tf=args.tf)
        seed = g._norm_params(_load_seed(base_symbol))

        # If seed doesn't have cycles, start permissive
        seed.setdefault("trade_cycles", [0, 1, 2, 3, 4])
        ...
        cfg = g.GAConfig(
            ...
            # now that cycles are free, trade counts should rise again
            min_trades_train=40,
            min_trades_val=15,
