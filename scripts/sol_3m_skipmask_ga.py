...
RUN_PREFIX = "SOL_3M_SKIPMASK_GA"
SYMBOL = "SOLUSDT"
BASELINE_STRATEGY_ID = "M1_ENTRY_ONLY_PASSIVE_BASELINE"
MEANINGFUL_WINNER_THRESHOLD = 0.0020
SKIP_FAMILY_ID = "TWO_CAP_OR"
SKIP_FEATURE_1 = "signal_range_pct"
SKIP_FEATURE_2 = "upper_wick_ratio"
...
def normalize_cfg(cfg: Dict[str, Any]) -> Dict[str, Any]:
    out = dict(cfg)
    # Freeze baseline execution mechanics; mutate only bounded causal skip-mask caps.
    mode = str(out.get("entry_mode", "limit")).strip().lower()
    out["entry_mode"] = "limit" if mode not in {"limit", "market"} else mode
    out["limit_offset_bps"] = float(out.get("limit_offset_bps", 0.75))
    out["fallback_to_market"] = int(1 if int(out.get("fallback_to_market", 1)) == 1 else 0)
    out["fallback_delay_min"] = float(out.get("fallback_delay_min", 6.0))
    out["max_fill_delay_min"] = float(out.get("max_fill_delay_min", 24.0))

    out["feature_skip_mask_enabled"] = int(1 if int(out.get("feature_skip_mask_enabled", 1)) == 1 else 0)
    out["skip_mask_logic"] = "or"
    out["skip_cap_signal_range_pct"] = float(np.clip(float(out.get("skip_cap_signal_range_pct", 0.08)), 0.01, 0.40))
    out["skip_cap_upper_wick_ratio"] = float(np.clip(float(out.get("skip_cap_upper_wick_ratio", 0.80)), 0.01, 0.99))
...
def random_cfg(rng: random.Random, baseline_cfg: Dict[str, Any]) -> Dict[str, Any]:
    cfg = dict(baseline_cfg)
    cfg = {
        **cfg,
        "feature_skip_mask_enabled": 1,
        "skip_mask_logic": "or",
        "skip_cap_signal_range_pct": rng.uniform(0.03, 0.20),
        "skip_cap_upper_wick_ratio": rng.uniform(0.30, 0.95),
        "candidate_id": "",
        "label": "GA random skipmask",
    }
...
def mutate_cfg(parent: Dict[str, Any], rng: random.Random, mutation_rate: float) -> Dict[str, Any]:
    ...
    if rng.random() < mutation_rate:
        c["skip_cap_signal_range_pct"] = float(c.get("skip_cap_signal_range_pct", 0.08)) + rng.gauss(0.0, 0.015)
    if rng.random() < mutation_rate:
        c["skip_cap_upper_wick_ratio"] = float(c.get("skip_cap_upper_wick_ratio", 0.80)) + rng.gauss(0.0, 0.06)
    c["candidate_id"] = ""
    c["label"] = "GA mutated skipmask"
    return normalize_cfg(c)


def crossover_cfg(a: Dict[str, Any], b: Dict[str, Any], rng: random.Random) -> Dict[str, Any]:
    out: Dict[str, Any] = dict(a)
    for k in ["skip_cap_signal_range_pct", "skip_cap_upper_wick_ratio"]:
        out[k] = a.get(k) if rng.random() < 0.5 else b.get(k)
    out["candidate_id"] = ""
    out["label"] = "GA crossover skipmask"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="SOL-only 3m bounded causal skip-mask GA under repaired frozen posture")
...
def main() -> None:
    ...
    baseline_cfg["skip_mask_logic"] = "or"
    ...
    # Discover persisted causal pre-entry features that are available at decision time.
    discovered_feature_cols = [
        c
        for c in [
            "feature_skip_signal_range_pct",
            "feature_skip_upper_wick_ratio",
        ]
        if c in baseline_full_rows.columns
    ]
    usable_feature_cols: List[Dict[str, Any]] = []
    for col in discovered_feature_cols:
        s = to_num(baseline_full_rows[col])
        coverage = float(s.notna().mean()) if len(s) else 0.0
        if coverage >= 0.95:
            usable_feature_cols.append({"column": str(col), "coverage": float(coverage)})
    if len(usable_feature_cols) < 2:
        raise RuntimeError(
            "Insufficient causal pre-entry features for skip-mask GA. "
            f"discovered={discovered_feature_cols}, usable={usable_feature_cols}"
        )
    ...
    def screen_candidate(cfg_in: Dict[str, Any], generation: int) -> CandidateEval:
        ...
        if h in screen_cache:
            ...
            row_cached = {
                ...
                "genome_hash": str(h),
                "feature_skip_mask_enabled": int(cfg.get("feature_skip_mask_enabled", 0)),
                "skip_mask_logic": str(cfg.get("skip_mask_logic", "or")),
                "skip_cap_signal_range_pct": float(cfg.get("skip_cap_signal_range_pct", np.nan)),
                "skip_cap_upper_wick_ratio": float(cfg.get("skip_cap_upper_wick_ratio", np.nan)),
                ...
                "same_bar_touch_count": int(ev.chrono["same_bar_touch_count"]),
    ...
    while len(population) < int(args.pop_size):
        population.append(random_cfg(rng, baseline_cfg=baseline_cfg))
    ...
    screen_df.to_csv(run_dir / "sol_skipmask_candidates_screen.csv", index=False)
    ...
    for h in top_hashes:
        ...
        if h in full_cache:
            ...
            holdout_loss = compute_loss_metrics(holdout_cand) if not holdout_cand.empty else {
                "instant_loser_rate": float("nan"),
                "bottom_decile_pnl_share": float("nan"),
            }
            holdout_base_loss = compute_loss_metrics(holdout_base) if not holdout_base.empty else {
                "instant_loser_rate": float("nan"),
                "bottom_decile_pnl_share": float("nan"),
            }
            holdout_instant_rel = relative_reduction(
                float(holdout_base_loss.get("instant_loser_rate", np.nan)),
                float(holdout_loss.get("instant_loser_rate", np.nan)),
            )
            holdout_tail_rel = relative_reduction(
            ...
            g4 = int(
                split_pass == 1
                and int(route_pack["route_support_pass"]) == 1
                and holdout_pass == 1
            ...
            full_eval_pack = {
                ...
                "holdout_loss_target_pass": int(holdout_loss_target_pass),
        ...
        confirm_rows.append(
            {
                ...
                "holdout_loss_target_pass": int(full_eval_pack["holdout_loss_target_pass"]),
                "holdout_instant_loser_rel_reduction": float(full_eval_pack["holdout_instant_loser_rel_reduction"]),
                "holdout_bottom_decile_rel_reduction": float(full_eval_pack["holdout_bottom_decile_rel_reduction"]),
                ...
                "same_bar_touch_count": int(ch["same_bar_touch_count"]),
    ...
    confirm_df.to_csv(run_dir / "sol_skipmask_candidates_confirm.csv", index=False)
    ...
    manifest = {
        ...
        "search_space": {
            "family_id": SKIP_FAMILY_ID,
            "feature_1": SKIP_FEATURE_1,
            "feature_2": SKIP_FEATURE_2,
            "skip_mask_logic": ["or"],
            "skip_cap_signal_range_pct": [0.01, 0.40],
            "skip_cap_upper_wick_ratio": [0.01, 0.99],
        },
        "gates": {
            "g0_chronology_clean": "parity_clean==1 and same_bar_exit_count==0 and exit_before_entry_count==0 and entry_on_signal_count==0",
            "same_bar_touch_note": "same_bar_touch_count is SL/TP tie-touch diagnostics only, not chronology violation",
            ...
            "g4_robustness": "split_valid_for_ranking==1 and best_min_subperiod_delta>0 and route_pass_rate==1.0 and holdout_delta_expectancy>0 and holdout_delta_cvar>=0 and holdout_loss_target_pass==1",
        ...
        "discovered_causal_preentry_features": {
            "from_signal_rows_columns": discovered_feature_cols,
            "usable_for_skipmask": usable_feature_cols,
            "selected_two_feature_family": [SKIP_FEATURE_1, SKIP_FEATURE_2],
        },
        "outputs": {
            "screen_csv": str(run_dir / "sol_skipmask_candidates_screen.csv"),
            "confirm_csv": str(run_dir / "sol_skipmask_candidates_confirm.csv"),
            "report_md": str(run_dir / "sol_skipmask_ga_report.md"),
    ...
    if decision == "APPROVED_EXEC_REPAIR":
        best_payload = {
            ...
            "candidate": {
                ...
                "feature_skip_mask_enabled": int(best_row["feature_skip_mask_enabled"]),
                "skip_mask_logic": str(best_row["skip_mask_logic"]),
                "skip_cap_signal_range_pct": float(best_row["skip_cap_signal_range_pct"]),
                "skip_cap_upper_wick_ratio": float(best_row["skip_cap_upper_wick_ratio"]),
            },
            "metrics": {
                ...
                "holdout_instant_loser_rel_reduction": float(best_row["holdout_instant_loser_rel_reduction"]),
                "holdout_bottom_decile_rel_reduction": float(best_row["holdout_bottom_decile_rel_reduction"]),
            },
            "provenance": {
                "run_dir": str(run_dir),
        ...
        json_dump(run_dir / "sol_skipmask_best_candidate.json", best_payload)
        ...
        [
            {
                ...
                "same_bar_touch_count": int(baseline_train_chrono["same_bar_touch_count"]),
            ...
            },
            {
                ...
                "same_bar_touch_count": int(baseline_full_chrono["same_bar_touch_count"]),
    ...
    report_lines: List[str] = [
        ...
        markdown_table(
            ...
            [
                "candidate_id",
                "feature_skip_mask_enabled",
                "skip_mask_logic",
                "skip_cap_signal_range_pct",
                "skip_cap_upper_wick_ratio",
                ...
                "holdout_loss_target_pass",
                "holdout_instant_loser_rel_reduction",
                "holdout_bottom_decile_rel_reduction",
                "confirm_pass",
            ],
            n=1,
        ),
        ...
        f"- Proven: bounded skip-mask family used 2 causal pre-entry features `{SKIP_FEATURE_1}` and `{SKIP_FEATURE_2}` only.",
    ...
    (run_dir / "sol_skipmask_ga_report.md").write_text("\n".join(report_lines), encoding="utf-8")
