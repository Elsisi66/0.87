...
def _simulate_1h_reference(
    *,
    ...
    for r in signals_df.itertuples(index=False):
        ...
        entry_time = pd.to_datetime(int(ts_ns[idx]), utc=True)
        entry_price = float(open_np[idx])
        sl = float(entry_price * sl_mult)
        tp = float(entry_price * tp_mult)
        eval_start_idx = int(idx + 1) if bool(defer_exit_to_next_bar) else int(idx)
            ...
            rows.append(
                {
                    **base,
                    ...
                    "entry_parent_bar_time": entry_time,
                    "exit_eval_start_time": pd.NaT,
                    "exit_eval_bar_time": pd.NaT,
                    "exit_time": entry_time,
                    "entry_price": float(entry_price),
                    "exit_price": float(entry_price),
                    "exit_reason": "no_full_bar_after_entry",
                    "mae_pct": np.nan,
                    "mfe_pct": np.nan,
                    "fill_delay_min": 0.0,
                    "hold_minutes": 0.0,
                    ...
                    "exit_slippage_bps": np.nan,
                    "total_cost_bps": np.nan,
                    "defer_exit_to_next_bar": int(bool(defer_exit_to_next_bar)),
                }
            )
            continue
        ...
        liq = "taker"
        exit_px = float(sim.get("exit_price", np.nan))
        if np.isfinite(entry_price) and np.isfinite(exit_px):
            c = _cost_row(float(entry_price), float(exit_px), liq, fee)
        else:
            c = {
                "pnl_gross_pct": np.nan,
                "pnl_net_pct": np.nan,
        ...
        eval_start_time = pd.to_datetime(int(ts_ns[eval_start_idx]), utc=True)
        rows.append(
            {
                **base,
                ...
                "entry_time": entry_time,
                "entry_parent_bar_time": entry_time,
                "exit_eval_start_time": eval_start_time,
                "exit_eval_bar_time": xt,
                "exit_time": xt,
                "entry_price": float(entry_price),
                ...
                "fill_delay_min": float((entry_time - st).total_seconds() / 60.0) if pd.notna(entry_time) else np.nan,
                "hold_minutes": float((xt - entry_time).total_seconds() / 60.0) if pd.notna(entry_time) and pd.notna(xt) else np.nan,
                "risk_pct": max(1e-8, 1.0 - sl_mult),
                "pnl_gross_pct": float(c["pnl_gross_pct"]),
                "pnl_net_pct": float(c["pnl_net_pct"]),
                "entry_fee_bps": float(c["entry_fee_bps"]),
